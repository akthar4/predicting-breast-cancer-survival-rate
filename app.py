...
# ----------------------------------------------------------------------------
# Styling. The interface chrome is deliberately neutral. One colour, a deep
# rose, is reserved exclusively for risk and never used for decoration, so that
# seeing it anywhere on the page always means the same thing. Awareness-campaign
# pink is avoided: this tool reports mortality to a clinician and should read as
# a medical instrument rather than a fundraising asset.
# ----------------------------------------------------------------------------
ROSE = "#993556"       # risk, and nothing else
ROSE_TINT = "#FBEAF0"
INK = "#2C2C2A"
MUTED = "#5F5E5A"
STONE = "#F1EFE8"
...
st.markdown(
    ...
    <style>
      ...
      .app-title {{font-size: 2.05rem; font-weight: 600; margin-bottom: 0.15rem;
                  color: {INK};}}
      .app-sub {{color: {MUTED}; font-size: 1.02rem; margin-bottom: 1.4rem;}}
      .panel {{background: {STONE}; border: 1px solid {STONE_LINE};
      ...
      .panel-title {{font-size: 0.78rem; font-weight: 600; letter-spacing: 0.08em;
                    text-transform: uppercase; color: {MUTED}; margin-bottom: 0.7rem;}}
      .riskvalue {{font-size: 3.4rem; font-weight: 600; line-height: 1; margin: 0;
                  color: {INK};}}
      .risklabel {{font-size: 0.95rem; color: {MUTED}; margin-top: 0.35rem;}}
      .banner {{border-radius: 10px; padding: 1rem 1.2rem; font-size: 1.02rem;
               margin: 0.4rem 0 1.1rem 0; border-left: 5px solid;}}
      .banner-flag {{background: {ROSE_TINT}; border-color: {ROSE}; color: #72243E;}}
      .banner-ok {{background: {STONE}; border-color: #888780; color: #444441;}}
      .track {{position: relative; height: 26px; background: linear-gradient(90deg,
              #E8E5DC 0%, #D8C3CB 40%, #C08296 72%, {ROSE} 100%);
      ...
      .dot {{position: absolute; top: 3px; width: 20px; height: 20px;
            background: #fff; border: 4px solid {INK}; border-radius: 50%;
            transform: translateX(-10px);}}
      .scale {{display: flex; justify-content: space-between; color: #888780;
              font-size: 0.76rem;}}
      .contrib-row {{display: flex; align-items: center; margin-bottom: 6px;
                    font-size: 0.86rem;}}
      .contrib-name {{width: 210px; color: #444441;}}
      .contrib-bar {{height: 15px; border-radius: 3px;}}
      .contrib-val {{margin-left: 8px; color: {MUTED}; font-size: 0.79rem;}}
      .footnote {{color: #6E6C65; font-size: 0.82rem; line-height: 1.55;}}
      div[data-testid="stMetricValue"] {{font-size: 1.45rem; color: {INK};}}

      /* Streamlit's own widget chrome. Themed here rather than in a
         .streamlit/config.toml so the whole application is one file. Several
         selector generations are covered because Streamlit renames its test ids
         between releases, and an unmatched selector simply does nothing. */
      ...
      button[kind="primary"]:hover {{
          background-color: #72243E !important;
          border-color: #72243E !important;
          color: #ffffff !important;
      }}
      section[data-testid="stSidebar"],
      div[data-testid="stSidebarContent"] {{
          background-color: #F5F3EE;
          border-right: 1px solid {STONE_LINE};
      }}
      div[data-baseweb="input"]:focus-within,
      div[data-baseweb="select"] > div:focus-within {{
          border-color: {ROSE} !important;
          box-shadow: 0 0 0 1px {ROSE} !important;
      }}
      div[role="radio"][aria-checked="true"] > div:first-child {{
          background-color: {ROSE} !important;
          border-color: {ROSE} !important;
      }}
      div[data-testid="stMetricDelta"] svg {{fill: {MUTED};}}
      div[data-testid="stMetricDelta"] {{color: {MUTED};}}
    </style>
    """,
...
for name, value in contrib.items():
    ...
    # Rose means risk. Anything that lowers risk is drawn in neutral stone.
    colour = ROSE if value > 0 else "#888780"
...
st.markdown(
    ...
    "average, on the model's log-odds scale. Rose pushes risk up, stone pushes "
    "it down. Note that examining more nodes reduces predicted risk once the "
